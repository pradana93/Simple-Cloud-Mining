...
class Users_model extends MY_Model
{
	...
	function __construct()
	{
		...
		//Logged in user ID
		$this->user_id = $this->ion_auth->user()->row()->id ?? null;
	}
	...
	{
		$this->db->select('user_plan_history.*,plans.plan_name as name, plans.id as plan_id');
		$this->db->join(
			'plans',
	...
	{
		return $this->db->where('user_id',$this->user_id)
	...
	function updateUserBalance()
	{
		...
		foreach($plans as $plan){
			...
			if($plan->expire_date !== null && date('Y-m-d H:i:s') >= $plan->expire_date){
				...
				//Update user balance
				$this->db->where('id',$this->user_id)->set('balance','balance+'.$earnings,FALSE)->update('users');
			}
		}
	}
	...
	function request_withdrawal()
	{
		...
		//Debit user balance
		return $this->db->where('id',$this->user_id)->set('balance','balance-'.$amount,FALSE)->update('users');
	}

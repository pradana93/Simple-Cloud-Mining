								...
								<?php if (!empty($plans)): ?>
									<?php foreach ($plans as $pl): ?>
										<tr>
											...
											<td><a href="<?php echo adminRoute('plans/edit/'.$pl['plan_id']); ?>"><?php echo $pl['name']; ?></a></td>
											<td>
												<?php
												if ($pl['status'] === 'active') {
											...
											<td><?php echo $pl['expire_date'] ?? 'Forever'; ?></td>
										</tr>
									<?php endforeach; ?>
								<?php else: ?>
									<tr>
							...
							<table class="table table-striped">
								...
								<?php if (!empty($withdrawals)): ?>
									<?php foreach ($withdrawals as $wt): ?>
										<tr>
											<td><a href="<?php echo adminRoute('withdrawals/edit/'.$wt['id']);?>"><?php echo $wt['id'];?></a></td>
											<td><?php echo ucfirst($wt['type']);?></td>
											<td><?php echo settings('currency_symbol');?> <?php echo $wt['amount'];?></td>
											<td>
												<?php
												...
												if($wt['status']==='PENDING'){
													echo "<span class='badge badge-warning'>".ucfirst($wt['status'])."</span>";
												}
												if($wt['status']==='PROCESSING'){
													echo "<span class='badge badge-info'>".ucfirst($wt['status'])."</span>";
												}
												if($wt['status']==='CANCELED'){
											...
											<td><?php if(isset($wt['tx'])): ?><a href="<?php echo blockchainUrl($wt['tx']);?>" target="_blank"><?php echo $wt['tx'];?></a><?php endif;?></td>
										</tr>
									<?php endforeach; ?>
								<?php else: ?>
									<tr>
								...
								<?php if(!empty($deposits)): ?>
                                        <?php foreach($deposits as $dep): ?>
                                            <tr>
                                                ...
                                                <td><?php echo settings('currency_symbol');?> <?php echo $dep['amount'];?></td>
                                                <td><?php echo $dep['tx'];?></td>
                                                <td>
                                                    <?php
								...
								<?php if(!empty($comissions)): ?>
									<?php foreach($comissions as $aff): ?>
										<tr>
											...
											<td><?php echo settings('currency_symbol');?> <?php echo $aff['amount'];?></td>
											<td>
												<?php
												if($aff['status']==='paid'){

...
class GatewayIpn extends MY_Controller
{
	...
	public function coinpayments()
	{
		...
		//Fields for detect conversion notifications
		$ipn_type = $this->input->post('ipn_type',true);
		$converted_to = $this->input->post('converted_to',true);

		//Ignore currency conversion notification
		if ($ipn_type == 'withdrawal' && isset($converted_to)) {
			die('IPN OK');
		}

...
class Withdrawals extends Admin_Controller
{
    ...
    {
		$results = $this->admin_paginate('withdrawals_model',adminRoute('withdrawals-history'), 'getAllPaid', ['status', 'SUCCESS']);
		$this->admin_view('withdrawals/list_paid', [
			'items' => $results['items'],
			'total_items' => $results['total'],
			'pagination_links' => $results['links'],
		]);
	...
	{
		$this->admin_view('withdrawals/view',[
			'item' => $this->withdrawals_model->getWithdrawalRequest($id)
		]);
	}
}
